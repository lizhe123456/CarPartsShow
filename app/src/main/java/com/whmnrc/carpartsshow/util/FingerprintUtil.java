...
import static com.whmnrc.carpartsshow.app.App.getInstance;
...
public class FingerprintUtil {
    ...
    public static  void callFingerPrint(final OnCallBackListenr listener){
        FingerprintManagerCompat managerCompat = FingerprintManagerCompat.from(getInstance().getmContext());
        if (!managerCompat.isHardwareDetected()){ //判断设备是否支持
            if (listener != null)
                listener.onSupportFailed();
            return;
        }
        KeyguardManager keyguardManager =(KeyguardManager) getInstance().getmContext().getSystemService(getInstance().getmContext().KEYGUARD_SERVICE);
